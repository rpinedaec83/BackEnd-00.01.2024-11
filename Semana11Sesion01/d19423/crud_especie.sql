...
DELIMITER $$
CREATE PROCEDURE crud_especie (in opcion int, in pdescripcion varchar(100), in pactivo bit,in pid int, in pusuario int)
BEGIN
	IF opcion = 1 then
		begin
			select * from especie where is_active = 1;
		end;
	...
	elseif opcion = 5 then
		begin
			select * from especie where id = pid;
		end;
    END IF;
END $$
DELIMITER ;